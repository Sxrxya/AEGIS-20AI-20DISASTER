        ...
        <div className="flex gap-2">
          {['Emergency', 'Warning', 'Watch'].map((severity) => (
            <Button
              ...
              className={cn(
                "h-9 px-4",
                filterSeverity === severity && severity === 'Emergency' && "bg-emergency hover:bg-emergency/90",
                filterSeverity === severity && severity === 'Warning' && "bg-warning hover:bg-warning/90"
              )}
            >
              {severity}
            </Button>
        ...
        <div className="lg:col-span-2 space-y-4">
          {isLoading ? (
            ...
            <Card className="bg-card/50 border-dashed border-2 flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground max-w-xs">Try adjusting your filters or search query.</p>
            </Card>
          ) : (
            filteredAlerts?.map((alert) => (
              <Card 
                ...
                className={cn(
                  ...
                  selectedAlert?.id === alert.id && "border-primary ring-1 ring-primary/20"
                )}
              ...
              >
                <CardContent className="p-0">
                  <div className="flex items-stretch">
                    <div className={cn(
                      "w-1.5 shrink-0",
                      alert.severity === 'Emergency' ? "bg-emergency" : 
                      alert.severity === 'Warning' ? "bg-warning" : "bg-blue-400"
                    )} />
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            alert.severity === 'Emergency' ? "text-emergency" : 
                            alert.severity === 'Warning' ? "text-warning" : "text-blue-400"
                          )}>
                            {alert.type === "Cyclone" ? <Wind className="h-5 w-5" /> : 
                             alert.type === "Flash Flood" ? <Waves className="h-5 w-5" /> : 
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="outline" className={cn(
                                "text-[10px] h-5",
                                alert.severity === 'Emergency' ? "border-emergency text-emergency" : 
                                alert.severity === 'Warning' ? "border-warning text-warning" : "border-blue-400 text-blue-400"
                              )}>
                                {alert.severity}
                              </Badge>
                            </div>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-border/50">
                        <div className="flex gap-6">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium">{alert.affectedPopulation.toLocaleString()} at risk</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <TrendingUp className="h-4 w-4 text-primary" />
                            <span className="text-xs font-medium">{Math.round(alert.confidence * 100)}% AI Confidence</span>
                          </div>
                        </div>
        ...
        <div className="space-y-6">
          {selectedAlert ? (
            <Card className="bg-card/50 border-border sticky top-24">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem className="gap-2"><Share2 className="h-4 w-4" /> Share Alert</DropdownMenuItem>
                      <DropdownMenuItem className="gap-2 text-emergency"><AlertTriangle className="h-4 w-4" /> Escalate</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ...
              <CardContent className="space-y-6">
                <div className="space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Recommended Actions</h4>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button className="w-full bg-emergency hover:bg-emergency/90">Dispatch</Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>Confirm Emergency Dispatch</DialogTitle>
                        <DialogDescription>
                          This will notify all emergency response units in the {selectedAlert.location} area.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="py-4 space-y-4">
                        <div className="p-4 bg-secondary/50 rounded-lg border border-border">
                          <p className="text-sm font-bold mb-1">Target Location</p>
                        ...
                        <div className="p-4 bg-emergency/10 rounded-lg border border-emergency/20">
                          <p className="text-sm font-bold text-emergency mb-1">Alert Type</p>
                          <p className="text-xs text-emergency/80">{selectedAlert.type} - {selectedAlert.severity} Level</p>
                        </div>
                      </div>
                      <DialogFooter>
            ...
            <Card className="bg-card/50 border-border border-dashed py-12 text-center">
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Select an alert from the list to view details and take action.</p>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="bg-card/50 border-border">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-muted-foreground">Units Deployed</span>
